...
var buffersPromise = new Promise(function (res) {return Tone.Buffer.on('load', res);});

var points = void 0,
notesOn = void 0,
startTime = void 0,
notesPlayed = _.times(loopDuration / step, function () {return _.times(gamut, function () {return 0;});});
...
function isLineRectangleIntersection(x1, y1, x2, y2, rx, ry, rw, rh) {
  ...
  var right = isLineLineIntersection(
  x1,
  y1,
  x2,
  y2,
  ...
  var bottom = isLineLineIntersection(
  x1,
  y1,
  x2,
  y2,
...
function detectPose(net, scaleFactor) {
  ...
  net.estimateSinglePose(captureCanvas, scaleFactor, true, 32).then(function (pose) {
    points = SEQUENCE.map(function (part) {return _.find(pose.keypoints, { part: part });}).filter(
    _.identity);
    ...
    for (var i = 0; i < steps; i++) {
      var x = i * noteWidth;
      var notesOnForStep = _.times(gamut, function () {return false;});
  ...
  setTimeout(function () {return detectPose(net, scaleFactor);}, step / 4 * 1000);
}

function getOffshootPoint(_ref, _ref2)


{var _ref$position = _ref.position,x1 = _ref$position.x,y1 = _ref$position.y;var _ref2$position = _ref2.position,x2 = _ref2$position.x,y2 = _ref2$position.y;
  if (x1 === x2 && y1 === y2) {
    return [x1, y1];
  } else if (x1 === x2) {
...
function render() {
  ...
  if (points) {
    ...
    /*if (points.length >= 2) {
      ...
      for (var _iterator = points[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {var point = _step.value;
...
startButton.addEventListener('click', function () {
  ...
  then(function (stream) {
    ...
    video.addEventListener('playing', function () {
      ...
      netPromise.then(function (net) {return detectPose(net, scaleFactor);});
